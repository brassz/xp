-- =====================================================
-- FIX COMPLETO - BANCO DE DADOS FRANCA PRIVATE
-- =====================================================
-- Este script cria todas as tabelas e estruturas ausentes
-- que estão causando erros 404 e 400 no sistema
-- =====================================================
...
-- =====================================================
-- Remover constraint antiga que permite apenas 'partial' e 'full'
-- para permitir valores mais descritivos como 'interest_renewal', etc.

-- Verificar se a tabela payments existe antes de alterar
DO $$ 
BEGIN
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'guarantors') THEN
        RAISE NOTICE '✓ Tabela guarantors criada com sucesso';
    END IF;
    ...
    RAISE NOTICE 'Todas as tabelas foram criadas e configuradas.';
...
-- 5. Atualize a aplicação web (F5)
-- 
-- Os erros 404 devem ser resolvidos após a execução deste script!
-- =====================================================
