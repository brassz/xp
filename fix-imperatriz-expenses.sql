...
BEGIN
    ...
    -- Criar índices novos com nome correto (se não existirem)
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_expenses_date') THEN
        CREATE INDEX idx_expenses_date ON expenses(date);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_expenses_user_date') THEN
        CREATE INDEX idx_expenses_user_date ON expenses(user_id, date DESC);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_expenses_category_date') THEN
