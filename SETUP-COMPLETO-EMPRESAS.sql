-- =====================================================
-- SETUP COMPLETO NEXUS - MÚLTIPLAS EMPRESAS
-- =====================================================
-- Script compilado para criar toda a estrutura do banco
-- Supabase para LITORAL CRED e MOGIANA CRED
--
-- INSTRUÇÕES:
-- 1. Acesse o SQL Editor do Supabase da empresa
...
-- =====================================================
-- PARTE 3: FOREIGN KEYS E CONSTRAINTS
-- =====================================================
...
-- Índices para documentos dos clientes
...
-- Índices para empréstimos quitados
...
-- Índices para empréstimos vencidos
...
-- Índices para empréstimos parcelados
...
-- Índices para empréstimos cancelados
...
-- Índices para parcelas individuais
...
-- Função para atualizar status do parcelamento automaticamente
...
-- Política para documentos dos clientes
...
-- Políticas similares para outras tabelas de status de empréstimos
...
-- Políticas para parcelas individuais
...
GRANT SELECT, INSERT, UPDATE, DELETE ON installment_payments TO authenticated;
...
-- Conceder permissões para views
GRANT SELECT ON loans_with_details TO authenticated;
GRANT SELECT ON overdue_loans_view TO authenticated;
GRANT SELECT ON financial_summary TO authenticated;
GRANT SELECT ON active_users TO authenticated;
GRANT SELECT ON emergency_contacts_with_client TO authenticated;
GRANT SELECT ON paid_loans_with_details TO authenticated;
GRANT SELECT ON cancelled_loans_with_details TO authenticated;
GRANT SELECT ON expenses_with_details TO authenticated;
GRANT SELECT ON expenses_summary_by_category TO authenticated;
GRANT SELECT ON current_month_expenses TO authenticated;
GRANT SELECT ON pending_approval_expenses TO authenticated;
...
AND table_name IN (
    'users', 'clients', 'guarantors', 'emergency_contacts', 'client_documents',
    'loans', 'payments', 'paid_loans', 'overdue_loans', 'partial_paid_loans', 
    'cancelled_loans', 'installments', 'installment_payments', 'expense_categories', 
    'expenses', 'cash_transactions', 'cash_settings', 'capital_raising', 
...
-- Verificar categorias de despesas inseridas
SELECT id, name, description, color, icon, is_active 
FROM expense_categories 
ORDER BY name;

-- Verificar clientes de exemplo
SELECT id, name, cpf, email, phone FROM clients ORDER BY name;

-- Verificar empréstimos de exemplo
SELECT l.id, c.name as client_name, l.amount, l.interest_rate, l.loan_date, l.due_date, l.status
FROM loans l
JOIN clients c ON l.client_id = c.id
...
-- 2. Cadastre novos usuários se necessário
